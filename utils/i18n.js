...
(function () {
  const I18N = {
    en: {
      ...
      generateBatch: 'Generate Title & Keywords ({n} pending)',
      generateBatchAll: 'Generate Title & Keywords (verify {n})',
      batchAborted: 'Batch aborted after repeated failures — {ok} done, {fail} failed, {skip} skipped.',
      ...
      batchChecking: 'checking the card keywords…',
    ...
    zh: {
      ...
      generateBatch: '生成标题和关键词（{n} 待处理）',
      generateBatchAll: '生成标题和关键词（全量校验 {n}）',
      batchAborted: '批量已中止（连续失败）—— 成功 {ok}，失败 {fail}，跳过 {skip}。',
      batchStop: '停止批量',
