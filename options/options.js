...
const OPT_I18N = {
  en: {
    ...
    optBatchProcessDesc: 'Adds a "Batch process" button to the panel. One click scans the Uploaded files grid for tiles marked with a red dot (missing title / keywords) and handles them one by one: select, generate, apply, save. If the red dot cannot be read from the page, the button switches to "verify all": it opens each tile and only processes the ones whose title / keywords are really missing (slower). Keep the Uploaded files page open while it runs — every asset is saved before moving on.',
  ...
  zh: {
    ...
    optBatchProcessDesc: '在面板上增加「批量处理」按钮：点一次就扫描「Uploaded files」网格里带红点（缺标题/关键词）的素材，逐个执行「切换素材 → 生成 → 应用 → 保存」。若页面的红点识别不到，按钮会自动变成「全量校验 N」：逐个打开素材检查，只处理真的缺标题/关键词的那些（较慢）。运行期间请保持停留在上传页，每张都会先保存再切下一张。',
